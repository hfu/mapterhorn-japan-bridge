...
| 1.6号 | `01M2EAPPYXT8RWNC6TXBRT36JE` (minted 2026-09-14) | **Next real launch (decided 2026-09-13, supersedes the earlier "1.7号" placeholder — 1.7号 stays unassigned)**. Same source data as 1.5号, plus (a) D165's own fixes including the live nodata/alpha bug, (b) land-area maxzoom upsampling (D149-151 design, corrected and implemented as D166 after a design review caught the original plan's `resolve_layer()` fix would have broken ~half the national downsampling pyramid). Code is implemented/design-reviewed/code-reviewed/tested against real 1.5号 data. D165's remaining 5 findings are all fixed and tested (D167, 2026-09-14, including a critical regression in #10's own fix caught by Opus code review before it ever ran). ID minted 2026-09-14 and added to `utils.LAND_UPSAMPLE_ZOOM_BY_GENERATION` in the same commit (D166 finding #3's own discipline). A small real dress-rehearsal (3 hand-picked real items: 1 sea-only reuse, 1 land upsample at the flagged 32768×32768px scale, 1 mixed coastal upsample) completed successfully 2026-09-14 (D168) — all behaved correctly, including one result that looked like a bug (100% nodata) but traced to correct behavior. **The full national dress rehearsal completed 2026-09-15 (D169)**: `aggregation_run.py` (6,373/6,373 items) + `downsampling_run.py` for both datatypes (8,415/8,415 items each), all against the real generation directory, zero errors across ~17 hours, disk headroom unchanged throughout (APFS COW clones made the ~4,245-item reuse-copy nearly free). Paused before `lineage_extend_low_zoom.py`/`bundle.py`/`merge_japan_bundles.py` (the steps that assemble the actual publishable archive) pending Hidenori's explicit go-ahead. Gated on: that decision, then the wet dress rehearsal per Hidenori's own sequencing. |

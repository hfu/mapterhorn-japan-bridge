...
| 1.7号 | *(not yet minted, planned 2026-09-24, D180/D182)* | **Same source data as 1.6号 again** (not a GSI data-update generation -- Hidenori's explicit naming decision, 2026-09-24: "2号はデータ更新を反映したときに使いたい", so this pipeline-only fix round gets its own label instead of overloading 2号, matching the 1.5号/1.6号 precedent of same-data-different-pipeline generations). Planned to bundle two independent, still-in-progress fixes once each is verified: (a) D180's coastal Gaussian-blur-seam fix (two independent Opus design reviews in progress, one complete as of this entry, recommends landing as a fresh generation rather than patching 1.6号 in place since the `.done` fingerprint mechanism won't auto-invalidate already-built items), (b) D182's z13+ wall extension (empirically confirmed necessary via a live check near Takeshima, 2026-09-24 -- the exact deferred question from D177). ULID not minted yet -- per this project's own standing discipline (D166 finding #3), minting happens in the same commit as any `utils.py` table entry it requires, not before either fix is actually ready to run. |

...
## D180追記2: 独立Opusレビュー(B)完了 -- レビューAと独立に、ほぼ同じ診断・ほぼ同じ修正案に到達。新たに「海面隆起」現象を発見、公開中の実タイルを直接デコードして裏付け

**Status**: レビューB(worktree分離、読み取り専用、実データで実行、217,794トークン・95ツール呼び出し・約30分)完了。レビューA・Bとも独立に同じ結論に収束 -- 設計の確信度は大きく上がった。

**レビューAとの主な一致点**: (1) D180の4つの実証的主張は全て再現、(2) ガウスぼかしが原因のほぼ100%でcubicsplineは無視できる(レビューB: RMS約0.09m)、(3) `boundary_tile`をvoid級/seam級に分割しseam側だけsigma上限をかける修正案、(4) D116の合成回帰テストでbit-identicalを実証、(5) 1.6号への現地パッチではなく新generation(1.7号)での導入を推奨、(6) D166が4,133/6,373アイテムをz16(sigma=30)化した点を問題悪化要因として指摘。

**レビューA未発見・レビューB独自の成果**:
- **公開中の実タイルを直接デコードして検証**(レビューAは再現コピーのみ): `pmtiles-store/aggregation/elevation/.../12-3649-1501-16.pmtiles`のタイル`16/58394/24024`を直接デコードし、独自再構築との一致を0.01m単位で確認。**実際に公開されているシリパ岬の断面で、実測113.29mの崖頂が69.22mとして配信され、実測1.43mの波食棚が39.50mとして配信されている**ことを直接確認。
- **「海面隆起」現象の発見**(D180にもレビューAにも無い視点): 海側(jpnationalseaが勝ったピクセル)にも陸地標高がにじみ出しており、**海面上に最大48.25mの「幽霊の陸地」が乗っている**。海岸から広い範囲で標高1m超の海面ピクセルが74,114px発見された。レビューBは「これが視覚的な『ゆるさ』印象の最大要因ではないか」と指摘。
- **「一方向のみ・値アンカー型ランプ」案を実際に試して却下**: 勝者側のピクセルは一切変更せず、埋め側だけをランプ化する設計を実装・実測したところ、110mの崖が海側に90m以上も引き伸ばされる結果になり(seam最大段差121.59m)、却下。「勝者を完全温存しつつ段差も消す、は原理的に不可能(段差は本物)」という重要な負の証拠。
- **4つの異なる海岸地形類型で実測**(シリパ岬=断崖、有明=干潟、三陸=リアス式で検証): リアス式海岸(DEM1Aが5.3%しか無い断片的地域)では改善幅が相対的に小さいという、正直な限界も報告。
- cap値として6(保守的、レビューAと同じ)、4(より鮮明、レビューAの推奨とも一致)を提示、両者ともσ=6かσ=4の二択に収束。

**両レビューが共に到達した推奨アルゴリズム**は、実装の変数名・微細な扱い(`never_covered_boundary`をdilationで導出する等)以外はほぼ同一 -- 独立した2つの調査が同じ場所に着地したことは、この設計の確信度を大きく高める。

**両レビューが共に明記する未検証事項**: (1) 実際のMapLibre 3Dビューアでの目視確認(両者ともヒルシェード近似のみ、本物の3Dレンダリングでは未確認)、(2) 47件のGLO-30欠損(void境界)アイテムのうち1件しか実データ検証していない、(3) cap=4か6かはHidenoriさんの目視判断が必要。

**次**: 両レビューの収束により設計自体の確信度は高い。実装 → D116回帰テスト → 実際のMapLibreビューアでの目視確認、という順で進める。ただし後述のD182追記(z13+の真のスコープ)がこの節の作業と並行して発覚したため、優先順位を要検討。

## D182追記1: z13+問題の真のスコープが竹島固有ではなく「日本近海のほぼ全外洋」であると判明 -- D174が当初「約8,600万タイル」という理由でz8-z12に縮小した懸念がそのまま復活

**Status**: 重大な発見、未対応。z13+拡張の「コスト」を再計算しようとした過程で、当初の前提(「GLO-30欠損地点だけの問題」)が誤りだと判明した。

**検証**: GLO-30欠損と無関係な、既知の欠損地域から離れた「普通の外洋」地点(北緯30.0度・東経140.0度)で同じチェックを行ったところ、**z12は200(正常)、z13以降は全て204**という、竹島と全く同じパターンが出た。

**原因**: `is_land_item_covering()`により、陸地を含まないz12マクロタイル(「海のみ」アイテム)は元々z13以降を生成しない設計(D149、「開けた海底に1m詳細の価値なし」という判断)。竹島近辺はこれに加えてGLO-30本家インベントリ自体が欠けている(D174の機序)が、**z13+で204になるという「見た目」は、GLO-30欠損の有無にかかわらず、陸地を含まない全てのz12マクロタイルで同じように起きる**。D175(MapLibre v6.11.1へのアップグレード、既に本番サイトへ反映済み)は、この一般的な「宣言されたmaxzoom内の204タイル」ケースには効いていないことも、D182の当初の目視確認(v6.11.1導入後のサイトで壁を再現)で実証済み。

**規模の実測**(`check_pmtiles_integrity.py`の実アーカイブ集計): z12タイル総数223,800に対し、z13タイル総数はわずか38,840 -- 概算で**21万枚前後のz12マクロタイルがz13以降を一切持たない**(「海のみ」で打ち止め)。これら全てにD174方式の合成フィルをz13-z16まで均等に広げると、4分岐×4段(z13→z16)で**7,000万〜9,000万タイル規模**になる -- D174の第一設計レビューが当初「z13-z16まで埋めると海全体で約8,600万タイル」として却下した、まさにその規模がそのまま復活する。

**対して、竹島のような「GLO-30本家インベントリ自体が欠けている」macrotileだけを対象にする狭いスコープ**(D177/D182がこれまで前提としてきたもの)は、実測中(z8-z12での対応する数字が463→1,825→7,221→28,657→114,101という等比的な伸びだったことから類推すると、桁で2〜3桁小さい規模と見込まれる、実測待ち)。

**これは戦略上の分岐点**: (a) 狭いスコープ(GLO-30欠損macrotileのみ、竹島のような特定地点を修復、安価)で進めるか、(b) 広いスコープ(全外洋、7,000万〜9,000万タイル規模、D174が最初に却下したのと同じ理由が正面から復活)に挑むか、(c) 別のアプローチ(例: アーカイブの宣言maxzoomを地域によって変える手段があるか、MapLibre/ビューア側で「宣言されたmaxzoom内でも204なら親タイルの値を使う」ような設定・挙動があるか、を先に調べる)を検討するか。Hidenoriさんの判断を仰ぐ。

## D182追記2: Hidenoriさんの判断「(b)広いスコープで」。実測により、(b)は当初推定より正確に定量化でき、かつ「同一内容タイルはpmtilesの重複排除でほぼ無償」というHidenoriさんの直感も裏付けられた。ただし(b)は2つの異なる安全根拠を持つ2つの部分から成ると判明

**Status**: スコープの精密な実測完了、実装は未着手。Hidenoriさんの指示: 「(b)じゃないかな。同じ空タイルならそれほど容量もとるまいね」。

**遅い逐次スキャン(前addendum時点でバックグラウンド実行中だったもの)を破棄し、`check_pmtiles_integrity.py`と同じ高速ディレクトリ走査(1回16秒)に切り替えて再計算**。結果:

```
z12 総タイル数:              223,800
z12 のうちz13実子を持つもの:    9,710
z12 のうちz13実子を持たない
  (=「葉」、埋め立て候補の親):  214,090

葉のうちGLO-30本家インベントリ
  自体が欠けている(D174の
  既存安全条件そのまま):      152,437   <- Review Bの「47件」推定は誤り、実際は桁違いに多い
葉のうちGLO-30本家データは
  実在する(＝本物の海、単に
  z12より深く作っていないだけ): 61,653

z12の葉1件あたりのz13-z16
  子孫タイル数(4^1+4^2+4^3+4^4): 340

【狭いスコープ】GLO-30欠損のみ:  51,828,580 タイル
【広いスコープ】全ての葉:        72,790,600 タイル
```

**重要な訂正**: 前addendumでは竹島のようなGLO-30欠損macrotileを「47件程度」(レビューBの推定)の小規模なものと想定していたが、実際にGLO-30本家インベントリ自体が欠けている葉は**152,437件**もある -- これは「竹島のような特定の孤島周辺」だけでなく、**この箱(116-160E/15-52N)の中の深い外洋の大半がそもそもGLO-30本家インベントリに含まれていない**ため(D174が既に確認済みの通り、GLO-30は陸地を含まない完全な外洋セルを最初から収録していない、24,674/64,800セルのみ)。つまり「狭いスコープ」を素直に全国規模へ広げるだけで、既にD174が最初に懸念した「8,600万タイル」規模の約6割(5,183万)に達する。

**Hidenoriさんのストレージに関する直感は正しい**: D174本番実績(z8-z12の152,267タイル→796ディレクトリエントリ、約191倍の重複排除)と同じ機構(同一52バイトのフラットタイルblobをHilbert曲線順の連続run-lengthとして1エントリに畳み込む)が働くため、7,200万タイルであっても実際のディスク増分はディレクトリエントリ分(せいぜい数十MB)+タイル本体1個(52バイト)程度に収まる見込み -- 273GBのアーカイブに対して無視できる規模。**容量は問題にならない**。

**しかし(b)は安全根拠の異なる2つの部分に分かれる**:
1. **狭いスコープ分(5,183万タイル)**: D174が既に確立・検証済みの「GLO-30本家インベントリに存在しない」という安全条件をそのまま使える -- 新しい設計判断は不要、ズーム範囲を12→16へ広げるだけ。
2. **広いスコープの残り分(2,096万タイル)**: GLO-30本家データは実在する(＝本物の海と確認済み)が、このプロジェクト自身のパイプラインがz12より深く作っていないだけの領域。ここは「GLO-30に無い」という安全条件では判定できない(GLO-30にデータがあるのでfill_eligible()はFalseを返す) -- **別の、しかし原理的には単純な安全根拠が必要**: この位置のz12親アイテムが、このプロジェクト自身のaggregation_merge.pyによって「jpnationalseaのみ(陸地ソース一切なし)」と既に判定済みであること(D174のGLO-30欠損チェックとは独立した、プロジェクト自身の優先マージ結果による確認)を条件にする。外国の陸地を誤って埋める心配がない(既に「陸地ソースなし」と確定済みの位置だけが対象のため)。

**次**: 2の安全根拠をどう正確にコード化するか(aggregation itemの`grouped_source_items`が`jpnationalsea`のみ、という条件を、z12「葉」の集合とどう対応付けるか)を設計し、`build_wall_fix_archive.py`にz13-z16対応の第三モードとして実装する。D180の修正(seam/void分類)とは独立の作業として並行できる。

## D182追記3: z13-z16の第三モードを実装・本番データでビルド完了。「グローバルGLO-30インベントリの存在」だけでは不十分と実装中に判明、jpnationalsea自身のダウンロード範囲を使う、より厳格な安全条件に修正

**Status**: Hidenoriさんの承認(「進めてよい」)を受けて実装。`hfu-mapterhorn/pipelines/build_wall_fix_archive.py`に`z13-z16`サブコマンドを追加。

**実装した安全条件の重要な訂正**: 実装中、addendum2で見積もった「GLO-30本家インベントリに存在する残り61,653件」をそのまま「広いスコープ」の対象にする案は不十分と判明した。グローバルなGLO-30インベントリ(24,674セル)には、日本近海の箱(116-160E/15-52N)内にある**韓国・中国・ロシア・台湾などの外国の陸地**も実在エントリとして含まれている——jpnationalseaが実際にダウンロードしたのはそのうち275セルだけであり、「グローバルインベントリに存在する」ことは「安全に埋めてよい」ことを意味しない。この区別を見落とすと、外国の陸地を誤ってフラットな0mで塗りつぶすリスクがあった。修正: 新しい`sea_fill_eligible()`関数を追加し、`source-store/jpnationalsea/bounds.csv`(このプロジェクト自身が実際にダウンロード・検証済みの275セル)とのみ照合するよう変更。

**モノトニシティによる高速化**: 子タイルの地理的footprint は常に親の部分集合であるため、親が安全条件(narrowまたはbroad)を満たせば、その340個の子孫(z13〜z16)全てが自動的に同じ条件を満たす——D174のtop-down parent-gatingと同じ不変条件を、逐次チェックではなく「適格な葉ごとに完全な部分木を一括生成する」ことで構造的に満たす設計とした。おかげで6,370万件の生成でも、実際の判定計算はz12の葉214,090件に対してのみで済んだ(15秒)。

**実測結果(本番`bundle-store/mapterhorn-japan-bridge.pmtiles`に対して実行)**:
```
z12総数: 223,800  z13実子あり: 9,710  葉(候補): 214,090
  narrow(GLO-30本家インベントリに不在、D174の条件そのまま): 152,437
  broad(jpnationalsea自身のダウンロード範囲に完全に収まる):   34,887
  除外(どちらでもない、外国または未確認領域の可能性):        26,766
z13-z16 fill合計: 63,690,160 タイル
```
10件の葉でのテスト(3,400タイル→40ディレクトリエントリ)で先に構造を検証してから本番実行。`bundle-store/wall-fix-z13-z16.pmtiles`として出力: **63,690,160個のアドレス化タイルが、わずか9,968ディレクトリエントリ・ファイルサイズ16KB**に収まった(`tile_contents_count=1`——全タイルが同一の52バイトblobを指す)。`pmtiles verify`クリーン。**Hidenoriさんの「同じ空タイルならそれほど容量もとるまいね」という直感が、実測でも完全に裏付けられた**。

**次**: `pmtiles merge`で本番アーカイブへ接合(実行中)、その後`check_pmtiles_integrity.py`で結合後のアーカイブ全体に孤児タイルが無いことを確認する。

**再現性の実測検証(Hidenoriさんからの質問「再生産をできる状態?」に対する回答)**: `build_wall_fix_archive.py z13-z16`を別出力先へ再実行し、MD5で比較したところ**一致しなかった**(`0ae35845...` vs `bc8c1297...`)。原因をバイト単位で特定したところ、16,086バイトの全体のうち差異はわずか4バイト(オフセット131-132と15763-15764)のみ、かつその位置はpmtilesの内部ディレクトリを包むgzipストリームのMTIMEフィールド(gzipヘッダのオフセット4-7)と厳密に一致した。実際にリトルエンディアンでデコードすると`1790252991`(2026-09-24 21:29:51)と`1790254533`(2026-09-24 21:55:33)——**それぞれの実行が実際に書き込みを完了した時刻そのもの**。つまり差異はgzip圧縮が自動的に埋め込む書き込み時刻のみで、**タイル位置・タイル内容・ディレクトリ構造は完全に同一** -- 生成ロジック自体は完全に決定論的であると実証された。

## D182追記4: 本番アーカイブへのマージ完了・全数検証クリーン・竹島の壁を実データで修復確認。starsへの公開はまだ行っていない

**Status**: `pmtiles merge`完了。`check_pmtiles_integrity.py`で67,311,837タイル全数を検証、孤児ゼロ。竹島近辺の元204タイルが実際に修復されたことをローカルアーカイブから直接デコードして確認。**stars公開はまだ実施していない**、別途明示的な承認を待つ。

**マージ結果**: `bundle-store/mapterhorn-japan-bridge.z13fix.pmtiles`(272,864,968,030バイト、元の272,864,950,554バイトから**約17KBの増分のみ**——6,370万タイル追加に対して無視できる増分、Hidenoriさんの直感通り)。`pmtiles verify`クリーン(313ms)。

**`check_pmtiles_integrity.py`による全数検証(67,311,837タイル、539秒)**:
```
z8: 903  z9: 3,550  z10: 14,091  z11: 56,088  z12: 223,800
z13: 788,136  z14: 3,152,544  z15: 12,610,176  z16: 50,440,704
addressed_tiles_count = 67,311,837 = 3,621,677(元) + 63,690,160(新規fill)、過不足なく一致
孤児チェック: CLEAN -- 全ズームで親タイルが存在
```
z13の内訳も裏取り: 実測788,136 = 元の実タイル38,840 + fill 4×187,324=749,296 で一致。z16も同様に50,440,704 = 2,485,760 + 256×187,324=47,954,944 で一致——算術的に完全に説明がつく、想定外の増減なし。

**竹島の壁、実データで修復確認**: マージ後のローカルアーカイブから、元々204だった5点(`13/7103/3176`、`13/7102/3176`、`14/14207/6352`、`15/28415/12704`、`16/56830/25408`)を直接デコード、全て52バイト・標高0.00mで正常に存在することを確認。

**無関係な実データの無傷確認**: 富士山山頂(z14)を修正前後のアーカイブ双方でデコードし、標高3773.9m・ファイルサイズ159,456バイトで完全に同一であることを確認 -- 意図しない副作用なし。

**現在の状態**: `bundle-store/mapterhorn-japan-bridge.z13fix.pmtiles`はローカルに存在するのみ。まだ`bundle-store/mapterhorn-japan-bridge.pmtiles`への差し替え(ローカルswap)も、starsへの公開も行っていない -- D177/D178の前例に倣い、ローカル差し替えとstars公開は別々の明示的な確認ポイントとして扱う。

## D180追記3: 修正を本番`aggregation_merge.py`へ実装。D116非退行(合成+実データ2件)・実データでの改善効果とも、両レビューの予測を正確に裏付け。ヒルシェード目視でも劇的な改善を確認

**Status**: Hidenoriさんの指示(「D180の実装に進めましょう」)を受けて実装。`hfu-mapterhorn/pipelines/utils.py`に`seam_blur_sigma_max = 6`を追加、`aggregation_merge.py`のブラー処理ブロックを、両レビュー(特にレビューB)が収束した設計(`never_covered_boundary`/`seam_boundary`への分割、両方を同じpre-blur合成から計算)で置き換えた。まだコミットはしていない(検証優先)。

**検証1: 合成テスト(D116自身のケースを直接再現、実際に変更後の`merge()`をimportして実行)**:
- CASE A(純粋なvoid境界、D116が対象としたケースそのもの): **bitwise identical、最大差分0.0**。
- CASE B(seam+void混在): void側は**bitwise identical**(未変化)、seam側(group0/group1境界)の実測誤差は平均39.15m→13.21mへ**66%削減**。

**検証2: 実データ(シリパ岬、`12-3649-1501-16`)で再現・レビューAの予測と正確に一致**: `aggregation_reproject.reproject()`で実際に再投影し、旧コード・新コードそれぞれで`merge()`を実行して比較:
```
                旧コード    新コード
mean誤差         0.228m      0.015m   (15倍改善)
p99誤差          7.34m       0.01m    (734倍改善)
max誤差          102.02m     87.32m
>10m改ざんpx数   238,479     10,456   (96%削減)
```
**レビューAが独自の再現ハーネスで予測した「cap=6での陸地px改ざん>10m: 10,456」と、実際に本番コードを実行した結果が完全に一致** -- 設計・予測の正確さが実証された。

**検証3(両レビューが「最重要な未検証事項」としていたT6): 実データで本物のvoid境界(D174のGLO-30欠損機序)がある実アイテムでの非退行確認**: レビューBが発見済みのアイテム`12-3500-1741-13`(never-covered 51.75%)を実際に再投影・マージし、旧コード・新コードを比較 -- **bitwise identical、最大差分0.0**。両レビューが「最も重要な残課題」としていた検証が完了し、クリーンだった。

**検証4: ヒルシェード目視確認(Hidenoriさんの元々の「確信ができてから」という基準への直接の回答)**: シリパ岬の崖付近をヒルシェードでレンダリングし、旧コード出力と新コード出力を並べて比較(`/tmp/d180impl/hillshade_before_after.png`、Hidenoriさんへ送付済み)。**旧コードは海岸線が特徴のない黒い塗りつぶしの塊になっているのに対し、新コードでは岩肌の質感・沖合の小島が明瞭に復元されている** -- 視覚的にも劇的な改善を確認。

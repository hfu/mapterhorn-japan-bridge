...
- **Disk headroom unchanged throughout**: 248GiB free at the very start (before any covering ran) and 248GiB free at the very end (after all four stages), despite the aggregation layer alone reaching 184GB (elevation) + 117MB (lineage) and the downsampling layer reaching 67GB (elevation) + 138MB (lineage). This is because `shutil.copy2()` on APFS (this project's filesystem, confirmed D156) automatically uses `fcopyfile`/clonefile semantics for same-volume copies -- the D163/D164 reuse mechanism's ~4,245 file copies cost essentially zero additional disk space, sharing blocks with 1.5号's own archive until/unless either side is later modified (neither generation's own pmtiles-store output is ever modified after being written, so this sharing should persist indefinitely). Worth noting explicitly for a future session sizing disk headroom for 2号 or any later generation: the "reused fraction" of a cross-generation build is nearly free in disk terms on this filesystem, which was not obvious going in and changes the risk calculus for D157-style ENOSPC incidents.

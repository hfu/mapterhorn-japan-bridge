...
**次**: 2の安全根拠をどう正確にコード化するか(aggregation itemの`grouped_source_items`が`jpnationalsea`のみ、という条件を、z12「葉」の집합とどう対応付けるか)を設計し、`build_wall_fix_archive.py`にz13-z16対応の第三モードとして実装する。D180の修正(seam/void分類)とは独立の作業として並行できる。

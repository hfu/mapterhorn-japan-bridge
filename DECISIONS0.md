...
covering **D0-D99**. See [DECISIONS.md](DECISIONS.md) for the
full index (all parts) and the field descriptions (Status/Context/
Decision/Consequences).

---

## D0: DECISIONS.mdを100件区切りでDECISIONS{N}.mdに分割、目次から形骸化したDate列を削除

**Status**: Accepted

**Context**: DECISIONS.mdがD131まで到達し、1ファイルとして肥大化していた。
目次(Table of contents)のDate列はD22以降ほぼ全て空欄になっており、さらに
D103以降の29件(D103-D131)は目次に行自体が存在しない状態だった――目次の
手動メンテナンスがエントリ数の増加に追いつかなくなっていたことの表れ。
Hidenoriさんより「日付列は空欄があるなら列ごと消してシンプルにする」
「ファイルが大きくなりすぎているので100個ずつくらいで分割してはどうか」
との指摘・提案を受けた。

**Decision**:
1. 目次からDate列を廃止し、`# | Title | Status`の3列構成にする。
2. 本体を100件区切りで`DECISIONS{N}.md`に分割する(`N = 番号 // 100`)。
   区切りを0始まりにするため、この分割そのものを記録する本エントリを
   D1の前に新設し**D0**とした――これにより`DECISIONS0.md`はD0-D99、
   `DECISIONS1.md`はD100-D199、という具合に、エントリ番号を100で割った
   商がそのままファイル番号と一致する(mod-100)。
3. `DECISIONS.md`自体は本文を持たず、目次だけを持つ索引ファイルに変わる。
   各行のリンクは`DECISIONS{N}.md#アンカー`を指す。

**Consequences**: 新しい決定を追加する際は、(1)対象の`DECISIONS{N}.md`に
本体を追記、(2)`DECISIONS.md`の目次に1行追加、の2箇所を更新する必要がある
(以前は1ファイル内で完結していた)。D54・D55は欠番(過去の整理で失われたか
統合された番号)だが、分割ロジックはこれをそのまま許容する(欠番があっても
`番号 // 100`で一意に決まるため)。次の分割境界はD200で、その時に
`DECISIONS2.md`を新設する。
